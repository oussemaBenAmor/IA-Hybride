...
Rappel pytest :
  - toute fonction nommée test_* est un test
  - le mot-clé `assert` vérifie une condition ; s'il échoue, le test échoue
  - @pytest.mark.parametrize répète un test avec plusieurs jeux de données
"""
import pytest
...
# ══════════════════════════════════════════════════════════════════════════════
# _drop_hallucinated_numbers : LA COUCHE 1 complète
# ══════════════════════════════════════════════════════════════════════════════

class TestDropHallucinatedNumbers:
    """Tests de bout en bout de la couche anti-hallucination.
    Ces tests ENCODENT les bugs réels T02 et T85 qu'on a corrigés."""
